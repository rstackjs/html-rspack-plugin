...
/**
 * Returns whether the given html already contains a charset meta tag.
 * Quoted attribute values are ignored to avoid treating content such as
 * `content="charset=utf-8"` as a charset declaration.
 *
 * @param {string} html
 * @returns {boolean}
 */
function hasCharsetMetaTag(html) {
  ...
  return metaTags.some((metaTag) =>
    /\scharset\s*=/i.test(metaTag.replace(/"[^"]*"|'[^']*'/g, '')),
  );
}
...
class HtmlRspackPlugin {
  ...
  postProcessHtml(
    ...
    if (this.options.inject) {
      ...
      const headStart = [];
      const headEnd = [];
      let hasCharset = hasCharsetMetaTag(html);
      ...
      for (const item of assetTags.headTags) {
        if (isCharsetMetaTag(item)) {
          ...
          // of the document, so inject it at the start of the head. Keep a charset
          // authored by the template and ignore additional generated declarations.
          if (!hasCharset) {
  ...
  generateFavicon(
    ...
    const inputFileSystem = /** @type {NonNullable<Compilation['inputFileSystem']>} */ (
