...
function template(string, interpolate = reInterpolate) {
  let isEscaping;
  let isEvaluating;
  let index = 0;
  let source = "__p += '";
  ...
  const reDelimiters = RegExp(
    reEscape.source + '|' + interpolate.source + '|' + reEvaluate.source + '|$',
    'g',
  );
...
function lodashEscape(input) {
  ...
  const { compiled, isEscaping } = template(source, options.interpolate);
  return `
